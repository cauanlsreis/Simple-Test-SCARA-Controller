...
Dicas de calibração
- Se a garra abre e fecha por ruído: aumente os thresholds (ex.: 0.05/0.14) ou aumente o intervalo mínimo entre comandos.
- Verifique iluminação e enquadramento da câmera: MediaPipe depende de boa visibilidade dos dedos.

Problemas conhecidos
- Alguns caracteres acentuados podem não aparecer corretamente na janela OpenCV em alguns sistemas; o README e textos na interface foram simplificados (sem cedilha) para evitar esse problema.
- Eixo Z foi removido temporariamente do firmware simplificado (havia problemas mecânicos/configurações). Restou apenas J1, J2 e Phi + Garra.

Contribuições
- Pull requests com melhorias na cinemática, filtros de gesto (ex.: filtragem temporal, debounce por voto) e integração com controle por joystick são bem-vindos.

Licença
- Coloque aqui a licença que desejar (por exemplo MIT) ou mantenha para uso pessoal.

---

Se quiser, eu atualizo também um pequeno arquivo de instruções de calibração/thresholds ou adiciono um script de ajuste automático baseado em amostras (ex.: medir distância média de pinça aberta/fechada). Quer que eu adicione isso? 
